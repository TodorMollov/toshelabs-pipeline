...
3. Trace callers ONLY for functions whose signature or behavior changes in a non-backward-compatible way. Pure additions (new fields, new logger calls, new tests) do NOT need callers traced — record them as "no_callers_needed: backward-compatible addition" in callers_traced.
   **TEST CALLERS MATTER.** If a function's signature changes (return type, async-ification, new required parameter), grep the test directories too — every test that calls the function must be updated, not just production code. Add affected test files to files_to_change with what_to_do = "update calls to use new signature (e.g. add the await keyword)" so the implement step actually edits them. Past pattern: BUG-253 failed tests_green because plan changed startSync from void to Future and listed only the production caller; existing tests in sync_test.dart called startSync without await, broke on next run, heal couldn't recover.
   **TYPE-IMPACT CHECK (cross-file blind spot — do this only when the change touches a type's shape).** Ask: does any planned change add a field, remove a field, or change a constructor signature on a type/class/struct/model T (including a Drift table/Entry, a data class, a DTO, a Riverpod state object)? If YES — and ONLY then — run ONE Grep for direct constructions of T (pattern \`T(\`) across BOTH production and test/fixture trees. Every file that constructs T directly must go into files_to_change with what_to_do prefixed \`[fixture-update]\` (e.g. "[fixture-update] add new required field \`foo\` with a sensible default to the T(...) call"). For a Drift table column add: also state that build_runner must regenerate generated code AND list every direct Entry/RowClass constructor in the test tree as [fixture-update] entries. If the ticket touches NO type signature, this check no-ops — do NOT emit any [fixture-update] entries (a plan with spurious fixture-update lines for an unchanged type is wrong). Rationale: the planner is routinely locally correct but globally incomplete — a new required field compiles in the source but breaks every untouched test fixture that constructs the type, and self-heal often cannot recover a fleet of compile errors.

...
After processing every finding, re-run the test suite + analyzer through the project's test runner `.claude/run-tests.sh` (see CLAUDE.md "Test execution") as **ONE foreground, blocking call**; read its exit code (0 = green). For fast checks while iterating you may scope it to paths (`.claude/run-tests.sh test/widget/foo_test.dart`), but the final pass before you proceed must be `.claude/run-tests.sh` with no args (full suite + analyzer). **NEVER** run `flutter test` directly, **NEVER** background a run or write an `until`/`while`/`pgrep` poll loop (one such loop once froze a run for 10.5h); raise the Bash timeout instead. They must be clean before you proceed. If a fix you applied broke tests, fix the regression (recursive small loop, max 3 iterations; if you can't converge, halt).
